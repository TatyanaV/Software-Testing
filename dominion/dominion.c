...
int card_Adventurer(struct gameState *state, int handPos)
{
  //current player
  int currentPlayer = whoseTurn(state);
...
int card_Feast(struct gameState *state, int choice1){
    //used for loop counter
      int x;
      int i;//used for loop counter
      //current player
      int currentPlayer = whoseTurn(state);
      //used for backup hand
      int temphand[MAX_HAND];
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case council_room:
      //+4 Cards
    card_Council_Room(state, handPos);
